...
export const ENV_REGISTRY = [
  ...
  { name: 'AUTOPLAY_ESCAPE_ENABLED', category: 'autoplay', type: 'bool01', defaultValue: '0', required: false, description: 'Enable temporary escape branches after basin quick-skip patterns.' },
  { name: 'AUTOPLAY_ESCAPE_T_MS', category: 'autoplay', type: 'int', min: 1000, max: 30000, required: false, description: 'Optional override for escape trial dwell threshold in milliseconds. If unset, derived from MUSIC_QUICK_SKIP_MS with a 30s cap.' },
  { name: 'AUTOPLAY_ESCAPE_COOLDOWN_SPAWNS', category: 'autoplay', type: 'int', min: 1, max: 10, defaultValue: '2', required: false, description: 'How many autoplay spawns stay in normal-continue mode after a failed D fallback.' },
  { name: 'AUTOPLAY_DISTINCT_ARTIST_SHORTLIST_ENABLED', category: 'autoplay', type: 'bool01', defaultValue: '0', required: false, description: 'Enable pre-ranker shortlist deduplication so only one known-artist candidate survives the first pass.' },
  { name: 'AUTOPLAY_DISTINCT_ARTIST_MIN_CANDIDATES', category: 'autoplay', type: 'int', min: 1, max: 20, defaultValue: '4', required: false, description: 'If the one-per-artist shortlist is smaller than this, relax the shortlist to allow limited repeats.' },
  { name: 'AUTOPLAY_DISTINCT_ARTIST_MAX_PER_ARTIST_FALLBACK', category: 'autoplay', type: 'int', min: 1, max: 4, defaultValue: '2', required: false, description: 'Per-artist cap used by the relaxed distinct shortlist fallback.' },
  { name: 'AUTOPLAY_ARTIST_QUARANTINE_ENABLED', category: 'autoplay', type: 'bool01', defaultValue: '0', required: false, description: 'Enable hard per-artist quarantine for quick-skipped tracks before candidate ranking.' },
  { name: 'AUTOPLAY_ARTIST_QUARANTINE_SPAWNS', category: 'autoplay', type: 'int', min: 1, max: 5, defaultValue: '1', required: false, description: 'How many autoplay spawn attempts quarantine a quick-skipped lead artist.' },
  ...
  { name: 'AUTOPLAY_VARIETY_CONTROLLER_ENABLED', category: 'autoplay', type: 'bool01', defaultValue: '1', required: false, description: 'Enable anti-repeat variety controller (default ON, set 0 to disable).' },
  ...
  { name: 'AUTOPLAY_ARTIST_COOLDOWN_ENABLED', category: 'autoplay', type: 'bool01', defaultValue: '1', required: false, description: 'Enable artist cooldown in candidate ranker (default ON, set 0 to disable).' },
