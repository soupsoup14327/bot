...
| Модуль | Строк | Что делает |
|---|---|---|
| `index.js` | ~365 | Клиент Discord, роутинг интеракций (slash / кнопки / модалки). Обработчик `voiceStateUpdate` (счётчик слушателей). Slash read-path `/likes` отдаёт эпhemeral-список избранного через `personal-signals.listLikes`. |
...
- SQLite остаётся основным dev/test path, а Postgres pack готовит parity для будущего PG CI/prod.
...
**3. CI matrix SQLite/Postgres**  
Generated migration packs уже разнесены по backend, но сам CI пока гоняет только SQLite-path. Следующий отдельный шаг — поднять Postgres job и проверить startup/migrate path на `postgres://...`.
...
- Нужно: подключить runtime history и дообучить мост по данным из БД
...
| CI matrix SQLite/Postgres | Средний | Backend-specific migration packs уже есть, но PG job ещё не включён |
